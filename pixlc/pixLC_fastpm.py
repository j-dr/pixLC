...
def write_to_cells_buff(catalog, outbase, chunksize, nchunks, 
                        startchunk, indexnside=16, lfilenside=1, 
    ...
    for fnum in range(nchunks):
        ...
        header[-3] = catalog['Header'].attrs['Omega0']
        header[-4] = catalog['Header'].attrs['MassTable'][1]
        header[-5] = catalog['Header'].attrs['BoxSize']
...
def map_LC_to_cells(nbodyfile, outpath, simlabel, rmin, rmax, lfilenside, rr0,
                    hfilenside=None, verbose=False, buffersize=None, cells=None,
                    chunksize=None):
    """
    Given a the root directory of the bigfile, write the particles to radial/hpix cells 
    breaking up higher radial bins into more refined healpix cells.
    
    nbodyfile -- root directory of bigfile
